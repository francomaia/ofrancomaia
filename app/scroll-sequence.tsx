...
export function ScrollSequence({ frames }: ScrollSequenceProps) {
  ...
  return (
    <section
      ref={sectionRef}
      id="inicio"
      className="hero-scroll relative min-h-[320vh]"
    ...
    >
      <div className="hero-sticky sticky top-0 min-h-screen overflow-hidden pb-10 pt-28 sm:pt-32">
        ...
        <div
          ...
          style={{
            opacity: 0.38 + progress * 0.48,
            transform: `translate3d(-50%, calc(-50% + ${progress * 24}px), 0)`,
          }}
        />
        ...
        <div className="site-shell relative min-h-[calc(100vh-9.5rem)]">
          <div className="relative z-30 flex flex-wrap items-center gap-5 pt-2">
            <span className="eyebrow">
              Sites · Branding · Identidade visual
            </span>
            <span className="text-[0.66rem] uppercase tracking-[0.18em] text-slate-500">
          ...
          <h1 className="pointer-events-none absolute inset-x-0 top-[18%] text-[clamp(4rem,10.8vw,10.4rem)] font-semibold leading-[0.78] tracking-[-0.082em] sm:top-[16%]">
            <span className="relative z-10 block max-w-[8.5ch]">
              Sites para marcas
            </span>
            <span className="outline-type relative z-30 ml-auto mt-[0.15em] block max-w-[9.8ch] text-right">
          ...
          <div
            ...
            style={{
              transform: `translate3d(calc(-50% + ${(progress - 0.5) * 8}px), 0, 0)`,
            }}
          >
            {frames.length ? (
              <div className="hero-wallpaper-stage absolute -inset-[4%]">
                {frames.map((frame, index) => {
                  ...
                  const blur = Math.min(18, distance * 15);
                  const offset = (index - framePosition) * 3.5;
                  return (
                    ...
                    <img
                      ...
                      style={{
                        opacity,
                        filter: `blur(${blur}px) saturate(${1.02 + opacity * 0.12}) brightness(${0.72 + opacity * 0.2})`,
                        transform: `scale(${1.025 + distance * 0.035 + progress * 0.02}) translate3d(${offset}%, ${(progress - 0.5) * (index % 2 ? -1.8 : 1.8)}%, 0)`,
                      }}
                    />
                  );
                })}
              ...
              <div className="absolute inset-x-[12%] bottom-0 top-[7%] rounded-[48%_48%_18%_18%/28%_28%_10%_10%] border border-dashed border-cyan-200/18 bg-cyan-300/[0.025]">
                <div className="absolute inset-0 grid place-items-center text-center">
                  <div>
                    <span className="text-7xl font-bold text-cyan-300/10">
                      3D
                    </span>
                    <p className="mt-3 text-xs uppercase tracking-[0.2em] text-cyan-100/35">
                      Personagem em produção
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div
              className="magic-particles absolute inset-0"
              aria-hidden="true"
            >
          ...
          <div className="absolute bottom-[1%] left-0 z-40 flex max-w-sm flex-col gap-6 sm:bottom-[4%]">
            <p className="text-base leading-7 text-slate-300 sm:text-lg sm:leading-8">
              Estratégia, interface e identidade reunidas em experiências
              digitais com presença própria.
            </p>
            <a
              href="#trabalhos"
              className={cn(
                buttonVariants({ variant: 'default', size: 'lg' }),
                'cyan-shadow h-12 w-fit rounded-full px-5 text-[0.92rem]',
              )}
            >
              Ver projetos{' '}
          ...
          <div className="absolute bottom-[2%] right-0 z-40 hidden items-center gap-3 text-[0.65rem] uppercase tracking-[0.18em] text-slate-600 sm:flex">
            ...
            <span className="h-px w-20 bg-white/10">
              <span
                className="block h-full bg-cyan-300 shadow-[0_0_10px_#46f2ff]"
          ...
          {frames[activeIndex] ? (
            <div
              key={activeIndex}
              className="scene-caption absolute bottom-[8%] right-0 z-40 hidden max-w-[17rem] text-right sm:block"
            >
              <span className="font-mono text-[0.68rem] uppercase tracking-[0.22em] text-cyan-300">
