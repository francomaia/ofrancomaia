...
async function applyAppMiddleware(options) {
	...
	if (forwarded.rewriteUrl) try {
		if (isExternalMiddlewareRewrite(forwarded.rewriteUrl, options.request)) {
			...
			return {
				kind: "response",
				response: await proxyExternalMiddlewareRewrite(options.externalRewriteRequest ?? options.request, forwarded.rewriteUrl, options.context)
			};
		}
	...
	if (!forwarded.applied) {
		...
		if (result.rewriteUrl) {
			...
			if (isExternalUrl(result.rewriteUrl)) {
				...
				return {
					kind: "response",
					response: await proxyExternalMiddlewareRewrite(options.externalRewriteRequest ?? options.request, result.rewriteUrl, options.context)
				};
			}
