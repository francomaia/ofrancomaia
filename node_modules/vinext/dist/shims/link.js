	...
	const runPrefetch = () => {
		(async () => {
			if (hasAppNavigationRuntime()) {
				...
				const autoPrefetch = mode === "auto" ? resolveAutoAppRoutePrefetch(rewrittenPrefetchHref ?? prefetchHref) : resolveFullAppRoutePrefetch();
				...
				const fetchPromise = autoPrefetch.cacheForNavigation && (gateViaRouteTree || gateViaLoadingShell) ? (async () => {
					...
					return scheduleAppPrefetchFetch((signal) => fetch(rscUrl, {
						headers,
						credentials: "include",
						priority,
						signal,
